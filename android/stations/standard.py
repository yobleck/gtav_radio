...
import os, re, subprocess; #subprocess.run(["play-audio",]);
...
def branching(input_list, input_dir, input_type):
    ...
    subprocess.run(["play-audio", input_dir + input_type + rand]);
    return rand;
...
def __play_radio(input_station, mode):
    ...
    while(playing):
        if(next_song): #play song dictated by intro at bottom of loop
            ...
            subprocess.run(["play-audio", station_dir + "songs/" + next_song]);
            next_song = None;
            ...
            subprocess.run(["play-audio", station_dir + "songs/" + rand_song]);
        ...
        if(rng > 0.5): #play another song
            ...
            subprocess.run(["play-audio", station_dir + "intro/" + rand_intro]);
