#oddball file for radio stations like self radio ifruit radio and los santos underground radio
#includes full_radio, no_ads_news, music_only
#TODO: modify everything to work with aforementioned stations
...
def __play_radio(input_station, mode):
    ...
    temp_mono_solo = list(mono_solo_ml); temp_intro = list(intro_ml); temp_general = list(general_ml); temp_id = list(id_ml); #this is same as above for all other lists
    temp_time = list(time_ml); temp_to = list(to_ml); temp_ad = list(ad_ml); temp_news = list(news_ml); 
    
    type_list = ["mono_solo/", "general/", "id/", "time/", "to/", "", "intro/"]; #for passing folder structure to branching func
    next_song = None;
    ...
    while(playing):
        ...
        if(rng > 0.5): #play another song
            ...
            if(rng < 0.375 and rng > 0.25 and station_input != "iFRUIT RADIO"): #play general
            ...
            if(input_station not in ["SELF RADIO","iFRUIT RADIO"]):
                ...
                branching(temp_intro, station_dir, type_list[6]);
