...
def main(stdscr):
    ...
    now_playing_scr.addnstr("Now Playing: ",now_playing_scr.getmaxyx()[1]); #f.readline()
    #f.close();
    ...
    while(running):
        ...
        if(x == 113 and "audio_thread" in locals()): 
            ...
            subprocess.run(["pkill", "-f", "play-audio"]);
        ...
        if(now_playing_counter % 100 == 0 and main_menu_in_focus): #massive number to avoid unnecessary file IO. see sleep
            ...
            now_playing_scr.addnstr(0,0,"Now Playing",now_playing_scr.getmaxyx()[1]); #f.readline()
            #f.close();
            now_playing_scr.refresh();
